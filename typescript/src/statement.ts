...
export function buildValueStatementPage(input: {
  ...
  for (const [index, posting] of ordered.entries()) {
    ...
    requireIdentity(posting.sourceNamespace, 'Value statement source namespace');
    ...
    if (!Number.isSafeInteger(posting.postingSequence) || posting.postingSequence < 1) {
    ...
    if (postingIds.has(posting.postingId) || accountSequences.has(posting.accountSequence)) {
      throw new Error('Value statement contains a duplicate posting identity or sequence');
    }
    ...
    if (amount === 0n) throw new Error('Value statement posting must be non-zero');
