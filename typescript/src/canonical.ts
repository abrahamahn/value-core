...
function canonicalize(value: unknown, ancestors: WeakSet<object>): unknown {
  ...
  if (typeof value === 'number' && (!Number.isFinite(value) || Object.is(value, -0))) {
    throw new Error('Canonical JSON numbers must be finite and cannot be negative zero');
  }
...
export async function domainSeparatedDigest(
  ...
  if (domain.trim().length === 0) throw new Error('Canonical digest domain is required');
  if (contractVersion.trim().length === 0) {
    throw new Error('Canonical digest contract version is required');
  }
  const cryptoView: { readonly crypto?: { readonly subtle?: SubtleCrypto } } = globalThis;
  if (cryptoView.crypto?.subtle === undefined) {
    throw new Error('Web Crypto SHA-256 is required for canonical value digests');
  }
