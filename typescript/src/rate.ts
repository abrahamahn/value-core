...
export function multiplyRationalHalfEven(
  ...
  if (amount < 0n || parsedNumerator <= 0n || parsedDenominator <= 0n) {
    throw new Error('Value rate requires a non-negative amount and positive rational terms');
  }
  ...
  const rounded =
    doubled > parsedDenominator || (doubled === parsedDenominator && quotient % 2n !== 0n)
      ? quotient + 1n
      : quotient;
...
export function createValueRateSnapshot(input: ValueRateSnapshotInput): ValueRateSnapshot {
  if (
  ...
  if (parseAmountMinor(input.numerator) <= 0n || parseAmountMinor(input.denominator) <= 0n) {
    throw new Error('Value rate numerator and denominator must be positive');
  }
  if (!Number.isSafeInteger(input.maxStalenessSeconds) || input.maxStalenessSeconds <= 0) {
    throw new Error('Value rate maximum staleness must be a positive safe integer');
  }
  const observedAt = parseRfc3339Millis(input.observedAt, 'Value rate observedAt');
  const recordedAt = parseRfc3339Millis(input.recordedAt, 'Value rate recordedAt');
  const effectiveAt = parseRfc3339Millis(input.effectiveAt, 'Value rate effectiveAt');
  if (observedAt > recordedAt || recordedAt > effectiveAt) {
    throw new Error('Value rate observed, recorded, and effective times are out of order');
  }
...
export function evaluateValueRateFreshness(input: {
  ...
  if (!Number.isSafeInteger(input.refreshIntervalSeconds) || input.refreshIntervalSeconds <= 0) {
    throw new Error('Value rate refresh interval must be a positive safe integer');
  }
  const capturedAt = parseRfc3339Millis(input.capturedAt, 'Value rate capturedAt');
  const evaluatedAt = parseRfc3339Millis(input.evaluatedAt, 'Value rate evaluatedAt');
  const ageMillis = evaluatedAt - capturedAt;
  if (ageMillis < 0) throw new Error('Value rate evaluation cannot precede its snapshot');
